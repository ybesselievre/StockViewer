...
app = dash.get_app()
dash.register_page(__name__, path = "/stocks", title = "Stocks evolution")
...
@app.callback(
    Output("stocks-dashboard", "children"),
    Input("memory", "data"),
)
def build_dashboard(is_pfm_computed):
    if is_pfm_computed:
        pfm = load_obj("./current_pfm")
        fig_list = pfm.get_stocks_figs()
...
layout = html.Div(
    ...
    children = [ 
        html.Div(id = "stocks-dashboard"),
        dcc.Loading( 
            id = "stocks-loading",
            children = [html.Div(id = "stocks-content", className= "stocks-content")],
